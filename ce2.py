...
class DigitalHumanUI(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        self.setStyleSheet("background: #FDFAFE;")#253 250 254
        self.asr_running = False
        self.asr_thread = None
        self._asr_wake = False
        self.busy = False
        self.is_playing_answer = False  # 新增：标记是否正在播放回答
    ...
    def play_video_frames(self, frames, audio_path, audio_duration):
        ...
        self.is_playing_answer = True  # 标记开始播放回答
    ...
    def _sync_frame_with_audio(self):
        ...
        if ms >= self.audio_total_ms - 20 or idx >= self.video_frame_count - 1:
            self.sync_timer.stop()
            # 添加延时处理
            QtCore.QTimer.singleShot(1000, self._on_playback_finished)

    def _on_playback_finished(self):
        self.is_playing_answer = False  # 标记播放结束
        self.idle_signal.emit()

    @QtCore.pyqtSlot(str)
    def on_asr_text(self, text):
        if self.busy or self.is_playing_answer:
    ...
    def start_asr(self):
        def asr_callback(text, wake_state):
            ...
            # 只在非播放状态且唤醒状态下处理语音
            if text and wake_state and not self.is_playing_answer and not self.busy:
